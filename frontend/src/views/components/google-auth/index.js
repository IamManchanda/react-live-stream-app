...
const GoogleAuth = class extends Component {
  ...
  handleAuthState(currentState) {
    (currentState) ? this.auth.signIn() : this.auth.signOut();
  }
  ...
  render() {
    ...
    return (
      <button 
        ...
        onClick={ () => this.handleAuthState(!hasSignedInState) }>
        ...
        { (!hasSignedInState) ? <span className="google-text">Login with Google</span> 
            : <span className="google-text">Logout your Account</span> }
      </button>
    );
  }
};
